...
Matrix normalizeInput(const Matrix &input, MODE mode) {
  ...
  const int mrank = max(org_rows, org_cols);

  Matrix output;
  output.resize(mrank, vector<int>(mrank, 0));

  int max_cost = 0;
  for (int i = 0; i < org_rows; i++) {
    for (int j = 0; j < org_cols; j++) {
      output[i][j] = input[i][j];
      max_cost = max(max_cost, output[i][j]);
    }
  }
...
Result Solve(const Matrix &input, MODE mode) {
  ...
  result.assignment.resize(mrank, vector<int>(mrank, NOT_ASSIGNED));
...
int calculate_distance_two_points(pair<int, int> & p1, pair<int, int> & p2){
...
int main(){
    ...
    while(cin >> N >> M){
        ...
        for (int i = 0; i<M; i++){
            ...
            cin >> couriers[i].second;
        }
        cin >> restaurant.first >> restaurant.second;
        
        Matrix distance_mat(N+M-1, vector<int> (N));
        for (int i = 0; i<M; i++){
            for (int j = 0; j<N; j++){
                distance_mat[i][j] = calculate_distance_two_points(bottles[j], couriers[i]) + calculate_distance_two_points(bottles[j], restaurant);
            }
        }
        for (int i = M; i<N+M-1; i++){
            for (int j = 0; j<N; j++){
                distance_mat[i][j] = calculate_distance_two_points(bottles[j], restaurant) * 2;
            }
        }
        if (DEBUG)
            PrintMatrix(distance_mat);

        Result r = Solve(distance_mat, MODE_MINIMIZE_COST);

        cout << r.totalCost << endl;
